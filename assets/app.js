(function () {
  ...
  function contractHistoryByYear(contracts) {
    const groups = new Map();
    contracts.forEach((contract) => {
      if (!groups.has(contract.year)) groups.set(contract.year, []);
      groups.get(contract.year).push(contract);
    });
    return [...groups.entries()].sort(([left], [right]) => right - left).map(([year, rows]) => {
      const total = rows.reduce((sum, contract) => sum + contract.price, 0);
      return `<section class="contract-year-group"><div class="contract-year-heading"><strong>${year}</strong><span>${integer.format(rows.length)} ${rows.length === 1 ? "contrato" : "contratos"}</span><b>${money.format(total)}</b></div><div class="parish-contract-list">${rows.map((contract) => `<a href="${contract.url}" target="_blank" rel="noreferrer"><span><b>${esc(contract.object || "Sem descrição")}</b><small>${esc(contract.suppliers.map((supplier) => supplier.name).join(" · ") || "Fornecedor não indicado")} · ${esc(contract.procedure || "Procedimento não indicado")}</small></span><strong>${money.format(contract.price)}<small>${esc(contract.date || String(contract.year))}</small></strong><i>↗</i></a>`).join("")}</div></section>`;
    }).join("");
  }
